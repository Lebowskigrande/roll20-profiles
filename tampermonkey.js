// ==UserScript==
// @name         Roll20 Overlay – Works with JSON (Fixed Spell Group Headers)
  ...
  return new Promise((resolve, reject) => {
    GM.xmlHttpRequest({
      ...
      onload: (res) => {
        ...
        catch (e) { reject(new Error("Failed to parse character JSON: " + e.message)); }
      },
...
(async function(){
  ...
  function iconHTML(type){ return ICON_SVGS[type||''] || ICON_SVGS.Force; }
  ...
  // ---- Slots persistence & helpers ----
  function ensureCur(obj = {}) { for (const lvl in obj) if (obj[lvl].cur == null) obj[lvl].cur = obj[lvl].max || 0; }
  ...
  function persistSlots(){
    GM_setValue('r20_slots', { spellSlots: characterData.spellSlots, pactSlots: characterData.pactSlots });
    window.dispatchEvent(new CustomEvent('r20Overlay:slots', { detail: characterData }));
  }
  function consumeSlot(kind, lvl){
    if (kind === 'spell'){ const s = getSpellSlotState(lvl); if (s.cur <= 0) return false; s.cur -= 1; }
    else { const p = getPactSlotState(lvl); if (p.cur <= 0) return false; p.cur -= 1; }
    persistSlots(); return true;
  }
  function rechargeSlot(kind, lvl){
    if (kind === 'spell'){ const s = getSpellSlotState(lvl); if (s.cur >= s.max) return false; s.cur += 1; }
    else { const p = getPactSlotState(lvl); if (p.cur >= p.max) return false; p.cur += 1; }
    persistSlots(); return true;
  }
  ...
  function createOverlay(){
    ...
    root.innerHTML = `
      <style>
        ...
        .card{ width:420px; border-radius:10px; background:#fff; border:1px solid #eee;
          ...
          opacity: 0.1; transition: opacity 250ms ease-in-out; }
        .card:hover { opacity:1; }

        .header{ display:grid; grid-template-columns:1fr auto; align-items:center; gap:8px;
        ...
        .quickbar{ display:flex; gap:6px; align-items:center; padding:4px 6px; background:#f9fafb; border-bottom:1px solid #eee; }
        .qbIcon{ height:17px; width:17px; display:block; cursor:pointer;
          -webkit-mask-image:url("https://i.imgur.com/pb0i97K.png"); -webkit-mask-repeat:no-repeat; -webkit-mask-position:center; -webkit-mask-size:contain;
          mask-image:url("https://i.imgur.com/pb0i97K.png"); mask-repeat:no-repeat; mask-position:center; mask-size:contain; background-color:#0c5e61; }
        .qbIcon:hover { background-color:#18b3b8; }
        .qbtn{ font-size:12px; font-weight:600; padding:3px 9px; border-radius:8px; border:1px solid #e5e7eb; background:#fff; cursor:pointer; }
        ...
        .blockHead{
          ...
          background: #edf7f7; border-bottom:1px solid #dfecec;
        }
        ...
        .tabBar .tab { flex:1; text-align:center; justify-content:center; }
        ...
        .tabPanel{ position:absolute; inset:0; padding:8px; overflow:auto;
          transform: translateX(100%); opacity:0; transition: transform 260ms cubic-bezier(.2,.8,.2,1), opacity 260ms linear; will-change: transform, opacity; }
        .tabPanel.active{ transform: translateX(0); opacity:1; z-index: 2; }
        ...
        #bodyPane{ overflow:hidden; transition: max-height 280ms cubic-bezier(.2,.8,.2,1); }
        ...
        .rows{ display:grid; }
        .row{ display:grid; grid-template-columns: 1fr minmax(56px,auto) minmax(64px,auto) 1fr; gap:6px; align-items:center; padding:6px 8px;
          border-bottom:1px solid #edf2f3; font-size:12px; line-height:1.1; border-left:2px solid transparent; border-right:2px solid transparent; cursor:pointer; }
        .row:nth-child(even){ background:#f7fbfb; }
        .row:last-child{ border-bottom:none; }
        .row:hover{ border-left-color:#0c5e61; border-right-color:#0c5e61; background:#ecf7f7; box-shadow: 0 0 0 1px #0c5e6122 inset; }
        ...
        .gmods{ display:flex; gap:6px; align-items:center; margin-bottom:6px; margin-top:2px; }
        .gmods select, .gmods label{ border:1px solid #ddd; border-radius:6px; background:#fff; font-size:11px; padding:2px 6px; font-weight:600; }
        .gmods input{ transform: scale(.9); }

        /* Features override */
        #featureList .row{ grid-template-columns: 75% auto auto auto; }
        #featureList .nameCell{ min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }

        .details{ grid-column: 1 / -1; max-height: 0; overflow: hidden; transition: max-height 220ms ease; font-size:11px; color:#374151; padding:0 8px; font-weight:400; white-space:pre-line; }
        ...
        /* Dots */
        .slotDot{ width:14px; height:14px; border-radius:999px; border:1px solid #dbe7e9; background:#dbe7e9; cursor:pointer; display:inline-block; transition: transform .05s, background .15s, border-color .15s; }
        .slotDot.filled{ background:#0c5e61; border-color:#0c5e61; }
        .slotDot.empty:hover{ box-shadow: 0 0 0 2px #0c5e611a inset; }
        .slotDot:active{ transform: scale(.96); }
        .dots{ display:flex; gap:8px; align-items:center; }

        /* SPELL GROUPS: header is inside each list (attached) */
        #spellGroups{ display:grid; gap:10px; }
        #spellGroups .spellList > header.groupHead{
          display:grid; grid-template-columns: 1fr auto; align-items:center; gap:10px;
          padding:6px 8px; background:#edf7f7; border-bottom:1px solid #e4eaec;
        }
        #spellGroups .ghLeft{ display:flex; gap:10px; align-items:center; min-width:0; }
        #spellGroups .ghRight{ display:flex; gap:8px; align-items:center; justify-content:flex-end; }
        #spellGroups .groupTitle{ font-size:12px; font-weight:600; color:#374151; white-space:nowrap; }

        /* Spell rows: two equal columns (Name | Info) */
        #spellGroups .row{ grid-template-columns: 1fr 1fr; }
        #spellGroups .row .nameCell{ min-width:0; }
        #spellGroups .row .meta{ min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

        /* Magic item rarity */
        .r-uncommon { color:#10b981; }
        .r-rare     { color:#3b82f6; }
        .r-veryrare { color:#a855f7; }
        .r-legend   { color:#f59e0b; }
        .r-common   { color:#9ca3af; }

        .mCharges{ display:flex; gap:6px; align-items:center; }
        .chargeDot{ width:6px; height:6px; border-radius:999px; border:1px solid #e5e7eb; background:#e5e7eb; cursor:pointer; }
        .chargeDot.filled{ background:#0c5e61; border-color:#0c5e61; }
      ...
      <div class="card" id="card">
        ...
        <style>
          ...
          .menu{ position:fixed; left:0; top:0; min-width:280px; max-width:360px; background:#fff; border:1px solid #e5e7eb; border-radius:10px; overflow:hidden; animation: menuIn 140ms cubic-bezier(.2,.8,.2,1); }
  ...
  function buildCompactDamageParts(a){
    ...
    let out = '';
    for (let i=0; i<terms.length; i++){ const t = terms[i]; out += (i===0) ? t : (t.startsWith('-') ? t : `+${t}`); }
    return out || '—';
  }
  ...
  function featureRowHTML(f,i){
    ...
    return `<div class="row featRow" data-feature="${i}" title="Click row to Use; expand for details">
      <div class="nameCell"><span class="semibold">${f.name}</span></div>
      <div></div><div>${counter}</div><div></div>
  ...
  // Spell row: 2 columns (equal width) Name | Info
  function spellRowHTML(sp) {
    return `<div class="row" data-spell-name="${sp.name}" title="Click to cast">
      <div class="nameCell"><span class="semibold">${sp.name}</span></div>
      <div class="meta">${sp.text || ""}</div>
    </div>`;
  }

  // -------- Grouped Spells: header INSIDE list --------
  function renderSpellGroups(){
    const byLevel = {};
    for (const sp of (characterData.spells || [])) {
      const lvl = sp.level ?? 0;
      (byLevel[lvl] ||= []).push(sp);
    }
    const slotLevels  = Object.keys(characterData.spellSlots || {}).map(Number);
    const spellLevels = Object.keys(byLevel).map(Number);
    const all = Array.from(new Set([0, ...slotLevels, ...spellLevels])).sort((a,b)=>a-b);

    const parts = [];
    for (const lvl of all) {
      const label = levelLabel(lvl);
      const s = lvl === 0 ? null : getSpellSlotState(String(lvl));
      const p = lvl === 0 ? null : getPactSlotState(String(lvl)); // pact dots appear in matching level header

      const headHTML = `
        <header class="groupHead" data-lvl="${lvl}">
          <div class="ghLeft">
            <div class="groupTitle">${label}</div>
            <div class="dots">
              ${s && s.max > 0 ? Array.from({length: s.max}, (_, i) => `
                <span class="slotDot ${i < s.cur ? 'filled':'empty'}"
                      data-kind="spell" data-lvl="${lvl}" data-index="${i}"
              ...
              <div class="dots">
                ${Array.from({length: p.max}, (_, i) => `
                  <span class="slotDot ${i < p.cur ? 'filled':'empty'}"
                        data-kind="pact" data-lvl="${lvl}" data-index="${i}"
                        title="${i < p.cur ? 'Click to consume' : 'Click to recharge'}"></span>
                `).join('')}
              </div>
            ` : ``}
          </div>
      ...
      const rows = (byLevel[lvl] || []).map(spellRowHTML).join('');

      parts.push(`
        <div class="list spellList" data-level="${lvl}">
          ${headHTML}
  ...
  function rerenderSpellGroups(root){
    const wrap = root.getElementById('spellGroups');
    if (wrap) wrap.innerHTML = renderSpellGroups();
  }
  ...
  function consumeMagicCharge(i){ const { cur } = getMagicCharges(i); if (cur <= 0) return false; setMagicCharges(i, cur - 1); return true; }
  function rechargeMagicCharge(i){ const { cur, max } = getMagicCharges(i); if (cur >= max) return false; setMagicCharges(i, cur + 1); return true; }
  function rarityClass(r){ r=(r||'common'); return r.includes('legend')?'r-legend':r.includes('very')?'r-veryrare':r==='rare'?'r-rare':r==='uncommon'?'r-uncommon':'r-common'; }
  ...
  function wireInteractions(root, host){
    ...
    const ro = new ResizeObserver(()=>{ if (GM_getValue('expanded',1)===1) bodyPane.style.maxHeight = bodyPane.scrollHeight + 'px'; });
    ro.observe(bodyPane);
    ...
    tabBar.addEventListener('click', (e)=>{
      ...
      next.style.opacity = '';
      const onDone = (ev)=>{ if (ev.target !== cur) return; cur.classList.remove('exit-left','exit-right'); cur.removeEventListener('transitionend', onDone); };
      cur.addEventListener('transitionend', onDone, { once:true });
    });

    // Init Spells UI
    hydrateSlotsFromStorage();
    rerenderSpellGroups(root);
    ...
    root.getElementById('gmShroud').value  = gm.shroud ?? 'off';
    root.getElementById('gmConvert').value = gm.convert ?? 'off';
    root.getElementById('gmBless').checked = !!gm.bless;
    root.getElementById('gmShroud').addEventListener('change', (e)=> setGlobalMods({ ...getGlobalMods(), shroud: e.target.value }));
    root.getElementById('gmConvert').addEventListener('change', (e)=> setGlobalMods({ ...getGlobalMods(), convert: e.target.value }));
    root.getElementById('gmBless').addEventListener('change', (e)=> setGlobalMods({ ...getGlobalMods(), bless: e.target.checked }));

    // Interactive child hover
    function isInteractiveChild(el){ return !!el.closest('.chip, .ctrlChip, .chargeDot'); }
    ...
    // SPELL & PACT header dots (inside list headers)
    root.addEventListener('click', (e)=>{
      ...
      if (kind !== 'spell' && kind !== 'pact') return;
      if (isFilled) consumeSlot(kind, lvl); else rechargeSlot(kind, lvl);
      rerenderSpellGroups(root);
    });
    ...
    // Spells (rows)
    root.getElementById('spellGroups').addEventListener('click', (e)=>{
      const row = e.target.closest('.row[data-spell-name]');
      if (!row) return;
    ...
    // Features simple interactions
    root.getElementById('featureList').addEventListener('click', (e)=>{
    ...
    // Inventory (magic) dots
    root.getElementById('magicList').addEventListener('click', (e)=>{
      ...
      if (dot){
        ...
        if(old){ const wrap=document.createElement('div'); wrap.innerHTML = magicRowHTML(characterData.inventory.magic[i], i); old.replaceWith(wrap.firstElementChild); }
        return;
      }
  ...
  function buildAtkdmgTemplate(attack, { adv = 'normal' } = {}) {
    ...
    const primaryType = (a.dmg1type ?? a.dmgType ?? 'Slashing');
    const hasDmg2 = Boolean(a.dmg2 && a.dmg2type);
    const secDice = hasDmg2 ? a.dmg2 : null;
    const secType = hasDmg2 ? String(a.dmg2type) : null;

    let shroudDice = null, shroudType = null;
    if (mods.shroud && mods.shroud !== 'off'){ shroudDice = '1d8'; shroudType = String(mods.shroud); }

    const addAbl   = a.addAblToDamage !== false;
    const dmgBonus = (addAbl ? ablMod : 0) + magic + (a.damageBonus ?? 0);
    const range    = a.range ?? '5 ft';
    const toType   = (mods.convert && mods.convert !== 'off') ? String(mods.convert) : null;
    const convType = (t)=> toType ? toType : t;

    const parts = [];
    parts.push({ dice: a.dmg1 ?? a.dice ?? '1d8', type: convType(primaryType) });
    ...
    const groups = {};
    for (const p of parts){ (groups[p.type] ||= []).push(p.dice); }
    ...
    const r1  = `[[1d20${atkBonusText}]]`, r2  = `[[1d20${atkBonusText}]]`;
  ...
  function openSmite(ev){
    const root = document.querySelector('#r20-overlay-host')?.shadowRoot;
    ...
    requestAnimationFrame(()=>{
      const r = menu.getBoundingClientRect(); const pad = 8;
