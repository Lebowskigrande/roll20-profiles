// ==UserScript==
// @name         Roll20 Overlay PRE-import
...
spells: [
  ...
  const getSave  = abl => (characterData.abilities[abl].mod||0) + (characterData.abilities[abl].saveProf ? characterData.profBonus : 0);
...
#featureList .nameCell{
  ...
  text-overflow: ellipsis;
      ...
      /* Spells — level groups, chips, and expandable rows */
.levelSection { border-top:1px solid #e5e7eb; }
.levelHead{
  display:flex; align-items:center; justify-content:space-between;
  padding:6px 8px; background:#edf7f7; border-bottom:1px solid #e4eaec;
  position: sticky; top: 0; z-index: 1;
}
.levelTitle{ font-weight:700; font-size:12px; color:#0f172a; }
.levelSlots{ display:flex; gap:10px; align-items:center; }
.levelSlots .slotGroupInline{ display:flex; gap:8px; align-items:center; }
.levelSlots .slotsTitle{ font-size:11px; color:#374151; }

.spellRow{
  display:grid; grid-template-columns: 1fr auto auto minmax(120px, 1fr);
  gap:6px; align-items:center;
  padding:6px 8px; border-bottom:1px solid #edf2f3; font-size:12px; cursor:pointer;
}
.spellRow:nth-child(even){ background:#f7fbfb; }
.spellRow:hover{
  border-left:2px solid #0c5e61; border-right:2px solid #0c5e61;
  background:#ecf7f7; box-shadow: 0 0 0 1px #0c5e6122 inset;
}
.spellName{ font-weight:700; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

.compChip{
  font-size:10px; border:1px solid #e5e7eb; background:#fff; padding:0 6px; border-radius:6px; font-weight:700;
  display:inline-flex; align-items:center; height:18px;
}
.compVSM{ letter-spacing:.5px; }
.compVSM .off{ opacity:.25; }
.tagSq{
  width:16px; height:16px; border:1px solid #e5e7eb; display:inline-grid; place-items:center;
  font-size:10px; font-weight:800; border-radius:4px; background:#fff;
}
.metaSmall{ color:#64748b; font-size:11px; }

.spellDetails{
  grid-column: 1 / -1;
  max-height:0; overflow:hidden;
  transition:max-height 220ms ease; padding:0 8px; font-size:11px; color:#374151;
}
.spellRow.open + .spellDetails{ padding-top:6px; }
...
function tagChipHTML(letter, title){
  return `<span class="tagSq" title="${title}">${letter}</span>`;
}

function renderLevelSlots(kind, level){
  // draws the dots for one “kind” at a specific level, if it exists
  const state = (kind === 'spell') ? characterData.spellSlots[level] : characterData.pactSlots[level];
  if (!state?.max) return '';
  // current state (from persisted values)
  const s = getSlotState(kind, String(level));
  const title = (kind==='spell') ? `Spell Slots (${level}${level===1?'st':level===2?'nd':level===3?'rd':'th'})`
                                 : `Pact Slots (${level}${level===1?'st':level===2?'nd':level===3?'rd':'th'})`;
...
function spellRowGroupHTML(level, spellsAtLevel){
  ...
  // decide which slots to show in this header
  ...
  const rows = spellsAtLevel.map((sp, idx) => {
...
function buildSpellGroupsHTML(){
  // group by level, starting with cantrips, skipping empty levels
...
function renderSpellsTab(root){
  ...
  function wireInteractions(root, host){
    ...
    function buildSmiteMsg({ useSpell, usePact, undead, prone, crit }){
      ...
      // Eldritch Smite (2nd): 1d8 + 1d8/level (2) = 3d8; crit doubles
      let eldDice = 0;
      if (usePact){
        eldDice = 1 + 2;
        if (crit) eldDice *= 2;
      ...
      if (usePact && prone)  notes.push("Attempt Knock Prone");
...
tabBar.addEventListener('click', (e)=>{
  ...
  cur.addEventListener('transitionend', onDone, { once:true });
    ...
    renderSlotsUI(root);
// Spells: slot dots (per-level headers)
  ...
  if (type === 'save'){
    ...
    root.getElementById('smiteRoll')?.addEventListener('click', ()=>{
      ...
      if (usePact){
        ...
        if (!ok) notes.push('No 2nd-level pact slots');
      }
      renderSlotsUI(root);
...
root.getElementById('spellGroups').addEventListener('click', (e)=>{
  ...
  // Toggle expand if clicking “empty” parts (not chips/dots)
  ...
  // Casting: left click anywhere on the row posts the spell to chat and consumes a slot if needed
  if (sp){
    let notes = [];
    if (sp.cast && sp.cast.kind && Number.isFinite(sp.cast.slotLevel)){
      const ok = consumeSlot(sp.cast.kind, String(sp.cast.slotLevel));
      if (!ok){
        notes.push(`No ${sp.cast.kind==='spell' ? `${sp.cast.slotLevel}rd`.replace('1rd','1st').replace('2rd','2nd') : `${sp.cast.slotLevel}rd`.replace('1rd','1st').replace('2rd','2nd')}‑level ${sp.cast.kind} slots`);
      }
      // re-render headers to reflect consumption/failure visual
      renderSpellsTab(root);
    }
