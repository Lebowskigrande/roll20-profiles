...
(function () {
  ...
  function GM_GetValue(k, d){ try { const v = GM_getValue(k, d); return v === undefined ? d : v; } catch { return d; } }
  ...
  // Saves: compute prof flags once per load (call this inside init after characterData exists)
  ...
  // Slots (persisted)
  function slotKey(kind, level){ return `slots_${kind}_${level}`; }
  ...
  function makeDraggable(host, handle){
    ...
    window.addEventListener('mouseup', ()=>{ if(!down) return; down=false; GM_SetValue('pos',{left:host.style.left, top:host.style.top}); });
  }
  ...
  window.Overlay = {
    init(root, host){
      ...
      // --- examples: wire in your existing renderers here ---
      // renderAbilities(root, cd, getSave);
      // renderAttacks(root, cd);
      // renderSkills(root, cd);
      // renderFeatures(root, cd);
      // renderMagic(root, cd);
      renderSlotsUI(root, cd);
      // renderSpellsTab(root, cd); // if you have this split by level
      ...
      new ResizeObserver(()=>{ if (GM_GetValue('expanded',1)===1) bodyPane.style.maxHeight = bodyPane.scrollHeight + 'px'; }).observe(bodyPane);
      ...
      // Example: attack list click handler (uses helpers, but no top-level DOM)
      ...
      root.getElementById('spellSlotsRow')?.addEventListener('click', (e)=>{
        ...
        if (dot.classList.contains('filled')) consumeSlot(cd, kind, lvl); else rechargeSlot(cd, kind, lvl);
        renderSlotsUI(root, cd);
      });
