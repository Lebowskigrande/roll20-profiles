// ==UserScript==
// @name         Roll20 Overlay – Works with JSON (Grouped Spells by Level + Header Dots)
...
(async function(){
  ...
  function ensureCur(obj = {}) {
    for (const lvl in obj) {
      if (obj[lvl].cur == null) obj[lvl].cur = obj[lvl].max || 0;
    }
  }
    ...
    root.innerHTML = `
      <style>
        ...
        .tabPanel{
          ...
          transition: transform 260ms cubic-bezier(.2,.8,.2,1), opacity 260ms linear;
        ...
        .row{
          display:grid; grid-template-columns: 1fr minmax(56px,auto) minmax(64px,auto) 1fr;
          gap:6px; align-items:center; padding:6px 8px;
          border-bottom:1px solid #edf2f3; font-size:12px; line-height:1.1;
        ...
        .row:hover{
          ...
          background:#ecf7f7; box-shadow: 0 0 0 1px #0c5e6122 inset;
        }
        ...
        /* Spell groups (one table per level, single header row) */
        #spellGroups { display: grid; gap: 10px; }
        .spellGroup  { display: grid; gap: 6px; }

        .groupHead{
          display: grid;
          grid-template-columns: 1fr auto; /* left: label + spell dots; right: pact dots (if any) */
          align-items: center;
          gap: 10px;
          padding: 6px 8px;
          background: #edf7f7;
        ...
        .ghLeft{ display:flex; gap:10px; align-items:center; min-width:0; }
        .ghRight{ display:flex; gap:8px; align-items:center; justify-content:flex-end; }
        .groupTitle{ font-size: 12px; font-weight: 600; color:#374151; white-space:nowrap; }

        /* Spell rows in groups: two equal columns (Name | Info) */
        ...
        /* Inline consumable charges — small dots */
      ...
      <div class="card" id="card">
        ...
        <style>
          ...
          .menu{
            position:fixed; left:0; top:0; min-width:280px; max-width:360px;
  ...
  function buildCompactDamageParts(a){
    ...
    for (let i=0; i<terms.length; i++){
      const t = terms[i];
      if (i === 0) out += t; else out += (t.startsWith('-') ? t : `+${t}`);
    }
    return out || '—';
  }
  ...
  // -------- Grouped Spells renderer (single header per group) --------
  function renderSpellGroups(){
    ...
    // All levels to show: 0 (Cantrips), any spell slot levels, any level that has spells
    ...
    for (const lvl of all) {
      ...
      const p = lvl === 0 ? null : getPactSlotState(String(lvl)); // pact dots on the right if this level matches pact level

      const headHTML = `
        <div class="groupHead" data-lvl="${lvl}">
          <div class="ghLeft">
      ...
      parts.push(`<div class="spellGroup" data-level="${lvl}">${headHTML}${listHTML}</div>`);
    }
    return parts.join('');
  }
  ...
  function wireInteractions(root, host){
    ...
    // Smart hover for interactive children
    ...
    // Click dots in group headers (SPELL & PACT)
    ...
    // Spells (rows: Name | Info) – cast by row click
    ...
    function updateOptionVisibility(){ optUndead.disabled = !optSpell.checked; optProne.disabled  = !optPact.checked; }
    ...
    root.getElementById('smiteRoll')?.addEventListener('click', ()=>{
      ...
      if (!useSpell && !usePact){ optSpell.checked = true; updateOptionVisibility(); return; }

      const notes = [];
      if (useSpell){
        const ok = consumeSlot('spell','1'); if (!ok) notes.push('No 1st-level spell slots');
      }
      if (usePact){
        const ok = consumeSlot('pact','3');  if (!ok) notes.push('No 3rd-level pact slots');
      }
      rerenderSpellGroups(root);

      let msg = buildSmiteMsg({ useSpell, usePact, undead, prone, crit });
      if (notes.length){ msg += ` {{range=${notes.join(" • ")}}}`; }
      sendToChat(msg); closeSmite();
    });
  }
